...
def compute_uv_risk(df: pd.DataFrame) -> pd.Series:
    ...
    for u in ["UVI","UV_INDEX","UV"]:
        if u in df.columns:
            ...
            direct = _logistic(score, k=5, x0=0.4)          # 시그모이드 보정
            break
    if direct is not None and direct.notna().all():
        return direct
    ...
    # 4) 시그모이드 보정
    fallback = _logistic(raw.clip(0,1), k=5, x0=0.5).fillna(0)
    return fallback if direct is None else direct.fillna(fallback)
