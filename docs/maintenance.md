...
현재 비활성화된 Tableau 태스크의 패키지는 `requirements/tableau.txt`로 분리한다.
`tableauserverclient>=0.26`이 요구하는 `urllib3>=2`는 Airflow 2.7.3 기본 provider와
호환되지 않으므로, Tableau 연동을 활성화할 때 Airflow 업그레이드 또는 별도 실행 환경을
먼저 결정한다.
...
- Airflow 2.7.3/Python 3.11/OpenJDK 17 이미지를 빌드하고 `pip check`를 통과했다.
  ...
  DAG import 오류 없음, 웹 health 응답, 컨테이너 내 unittest 52개(50개 통과, 2개 건너뜀)를
  확인했다.
