...
V5 전환 후 일부 미지원 지역에서 반환되는 `resultCode: "99"`(검색결과 없음)를 결측으로 처리하고,
인천 등 세부 구 코드가 없는 지역은 광역시도 코드로 1회 폴백 및 캐싱하도록 보완했다.
실제 API로 전국 247개 행정구역에 대해 `fetch_and_save_uv_wide` 정상 수집(220개 지점 UVI 생성,
미지원 지역 경고 스킵)을 검증했다.
