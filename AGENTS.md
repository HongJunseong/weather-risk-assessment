...
- Python 기상 데이터 파이프라인이다. `docs/architecture.md`에서 역할과 데이터 흐름을, `docs/maintenance.md`에서 현재 제약을 확인한다.
...
- 외부 API·S3·Slack 없이 가능한 검증부터 수행한다. 기본 검증: `python3 -m unittest discover -s tests -v`, `python3 -m compileall -q weather_risk_assessment dags tests`.
...
- 하나의 검토 가능한 작업이 완료되고 관련 검증이 통과하면 `docs/commit-conventions.md`에 따라 로컬 커밋을 만든다. 별도 요청 없이 push, merge, rebase, amend, tag 생성은 하지 않는다.
